...
{
public:
    explicit HSpinnerPrivate(HSpinner *qq);
    ...
    Q_DECLARE_PUBLIC(HSpinner);
private:
    HSpinner* q_ptr;
};

HSpinnerPrivate::HSpinnerPrivate(HSpinner *qq)
    : q_ptr(qq)
{
...
/*!
    \~chinese \class HSpinner
    \~chinese \brief 可以使用 HSpinner 类快速创建用于指示加载中状态的旋转等待图标动画控件。

    \~chinese \image html HSpinner.gif

    \~chinese HSpinner 提供了一个用于指示加载中状态的旋转等待图标动画控件。在创建图标后，使用 start() 即可开始图标旋转的动画。
*/

/*!
    \~english \class HSpinner
    \~english \brief Use HSpinner to create a widget with spinning animation for display a waiting state.

    \~english \image html HSpinner.gif

    \~english HSpinner provide a spinning animation widget to indicate a waiting state. Call start() to start
    the spinning animation.
*/

/*!
 * \~english \brief Create a HSpinner widget
 * \~chinese \brief 创建一个 HSpinner 控件
 *
 * \~english \param parent Parent widget.
 * \~chinese \param parent 父控件
 */
HSpinner::HSpinner(QWidget *parent) :
    QWidget(parent), d_ptr(new HSpinnerPrivate(this))
{
    Q_D(HSpinner);
...
/*!
 * \~english \brief Is the HSpinner spinning or not
 * \~chinese \brief HSpinner 是否正在播放旋转动画
 */
bool HSpinner::isPlaying() const
{
    Q_D(const HSpinner);
 ...
 */
void HSpinner::setBackgroundColor(QColor color)
{
...
void HSpinner::paintEvent(QPaintEvent *)
{
    Q_D(HSpinner);
...
//            qDebug() << "color" << palette().highlight().color();
        for (int i = 0; i < 3; ++i)
...
void HSpinner::changeEvent(QEvent *e)
{
    Q_D(HSpinner);
...
QList<QColor> HSpinnerPrivate::createDefaultIndicatorColorList(QColor color)
{
    QList<QColor> colors;
    QList<int> opacitys;
