#ifndef EXTRACT_DJ1_H
#define EXTRACT_DJ1_H
#include "../abstractpart.h"
#include <QVector>
#include <QMetaObject>
class Extract_DJ1 : public QObject
{
    Q_OBJECT
...
public:
    ...
    Q_INVOKABLE int move(RackMotionParams param);
    ...
    int cleaning_liqud_clean_start(int cleaning_liquid_clean_time, int pure_water_clean_time, float z_position);
    int open_little_inside_valve();
    int close_little_inside_valve();
    int open_big_inside_valve();
    int close_big_inside_valve();
    ...
    //参数
    int getParameter(ExtractDj1Params& params);
    int setParameter(const ExtractDj1Params& params);
};
