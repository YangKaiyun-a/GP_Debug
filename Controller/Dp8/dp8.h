...
public:
    ...
    //模块参数
    struct Dp8_jawParams {
        float zFlyPosition;
        float jawZDownPosition;
        float z_speed;
        float claw_z_speed;
        float claw_z_zero_offset;
    };
    ...
    struct testparam{
        ...
        float tip_start_positon;
        float tip_stop_positon;
...
public:
    ...
    Q_INVOKABLE void test(Dp8::testparam param, int is_pause);
    ...
    //参数
    int getParameter(Dp8_jawParams& params);
    int setParameter(const Dp8_jawParams& params);
